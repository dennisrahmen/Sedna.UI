...
internal static class CataloguePages
{
    ...
    [
        ...
        new("/button", Actions, "Buttons", "ri-cursor-line",
            ...
            "btn primary go warn danger secret disabled icon link ghost fab floating small large busy pending aria-busy loading spinner in flight"),
        ...
        new("/form", Forms, "Form layout", "ri-edit-box-line",
            ...
            "form-field form-label form-hint sedna-grid field-grid two-up fieldset legend section actions split start required settings card assemble submit whole"),
        ...
        new("/form-datetime", Forms, "Date and time", "ri-calendar-line",
            "Date, time, month and week inputs, a from/to pair, and what iOS does to them.",
            "date time datetime-local month week calendar picker range from to min max step iphone ios stretch"),
        ...
        new("/form-numeric", Forms, "Ranges and steppers", "ri-number-1",
            ...
            "range slider stepper number spinner appearance disabled"),
        new("/form-color", Forms, "Colour", "ri-palette-line",
            "The platform's colour picker in the field's chrome, and a preset palette of swatches.",
            "color colour picker form-color swatch swatch-grid palette aria-pressed label category"),
        ...
        new("/files", Content, "Files", "ri-folder-3-line",
            "A folder as a table or a grid of tiles, attachments, and uploads with their progress.",
            "file files folder document attachment upload progress failed retry download type icon extension middle ellipsis truncate name size owner modified breadcrumb path tile thumbnail select selection empty folder drive"),
        new("/video", Content, "Video", "ri-film-line",
            "A frame around the native player with its title, length and transcript, and video tiles.",
            "video player movie clip recording training poster duration captions subtitles transcript track vtt tile thumbnail grid"),
        new("/audio", Content, "Audio", "ri-music-2-line",
            "A player around the native audio element, a waveform for a recording, and a track list.",
            "audio sound player recording call voicemail voice message podcast waveform wave playlist track list transcript"),
        ...
        new("/editor", Content, "Rich-text editor chrome", "ri-edit-box-line",
            "The frame, toolbar and body for an editor your app brings.",
            "editor editor-toolbar editor-sep editor-body rich text wysiwyg tiptap quill prosemirror contenteditable toolbar aria-pressed"),
        ...
        new("/progress", Feedback, "Progress and spinners", "ri-loader-4-line",
            "Determinate bars, indeterminate bars, and spinners.",
            "progress bar spinner loading busy indeterminate"),
        new("/skeleton", Feedback, "Skeletons", "ri-layout-masonry-line",
            "Placeholders the shape of what is coming: lines, cards, rows, tables, forms, tiles, a chart.",
            "skeleton placeholder loading shimmer aria-busy lines card list table form stat chart image block height"),
        ...
        new("/modal", Overlays, "Modal", "ri-window-2-line",
            ...
            "dialog backdrop confirm confirmation returnValue showModal sm lg scroll tall max-height footer ShowModalAsync CloseModalAsync deck first-run"),
        new("/drawer", Overlays, "Drawers and sheets", "ri-layout-right-line",
            "A panel from an edge, for a secondary flow that can be left.",
            "drawer sheet side panel filter edge scrim ISednaOverlays SednaOverlay SednaOverlayHost ShowAsync overlay component result"),
        ...
        new("/spotlight", Overlays, "Spotlight", "ri-focus-3-line",
            ...
            "spotlight tour onboarding hint highlight hole walkthrough follow lock placement coach mark FollowSpotlightAsync SednaSpotlight SpotlightOptions"),
        ...
        new("/script", Start, "The script", "ri-code-s-slash-line",
            "Loading it, and the JavaScript behind toasts, the clipboard and the delegated behaviours.",
            "sednaUi toast modal copy clipboard menu tabs dropzone follow javascript boot theme default time zone cookie tz select refresh selectedcontent",
            Section: Reference),
        new("/interop", Start, "From C#", "ri-braces-line",
            "ISednaUi: toasts, dialogs, settings, the time zone and the spotlight, called from a component.",
            "ISednaUi interop C# csharp AddSednaUi ToastAsync ShowModalAsync ISednaOverlays FollowSpotlightAsync ISednaSettings LoadSettingsAsync GetTimeZoneAsync ShowModalAsync prerender OnAfterRenderAsync",
            Section: Reference),
